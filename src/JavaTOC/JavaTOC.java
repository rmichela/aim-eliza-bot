...
public class JavaTOC {
   ...
   public void send(String to,String msg)
   {
     try {
       ...
       if(msg.indexOf("<slap:anon>") != -1)
       {
         this.warn(to, true);
       }
       if(msg.indexOf("<slap:norm>") != -1)
       {
         this.warn(to, false);
       }
    ...
    * Send a warning
    *
    * @param to Screen name to send a warning to
    * @param anonymous Switch for anonymous warnings
    */
   public void warn(String to, boolean anonymous)
   {
     try {
      if(anonymous)
      {
       this.sendFlap(DATA,"toc_evil " + normalize(to) + " \\\"anon\\\"");
      }
      else
      {
        this.sendFlap(DATA,"toc_evil " + normalize(to) + " \\\"norm\\\"");
      }
     } catch ( java.io.IOException e ) {
     }
   }
